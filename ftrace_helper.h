...
#define HOOK2(_name, _hook, _orig)   \
{                   \
    .name = (_name),        \
    .function = (_hook),        \
    .original = (_orig),        \
...
#if defined(CONFIG_X86_64) && LINUX_VERSION_CODE >= KERNEL_VERSION(5,7,0)
// 声明一个函数指针，用来存放 kallsyms_lookup_name 的地址
...
static int fh_resolve_hook_address(struct ftrace_hook *hook)
{
    ...
    if (ret < 0) {
     ...
     // kprobe.addr 就是 kallsyms_lookup_name 在内核中的实际地址
...
#if defined(FTRACE_REGS_SYSCALL_STUBS)
